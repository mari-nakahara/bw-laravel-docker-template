...
コードの可読性と保守性を向上させる
メソッドチェーンを使用することで単一の文で、同一オブジェクトの複数のメソッド呼出すことができる。
...
LaravelはなにでINSERTとUPDATEを判断しているか？
まだDBに保存していない → 新規（INSERT） 
DBから取得したデータ → 既存 (UPATE)
...
バージョン管理できる
チーム開発でもDB構造が共有できる
デプロイ時にも自動で構築できる(アプリをサーバーに配置したときに、必要な準備が自動で完了)
...
CSRF対策なしではなりすましや悪意のあるPOSTリクエストを偽造することできる。
@csrf を入れるだけで、Laravelはそれを自動でブロックする。
